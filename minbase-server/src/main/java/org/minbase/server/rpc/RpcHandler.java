    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, RpcProto.RpcRequest rpcRequest) throws Exception {
        ...
        try {
            ...
            if (callType == CallType.CLIENT_GET.getType()) {
                ...
                ClientProto.DeleteRequest deleteRequest = ClientProto.DeleteRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.DeleteResponse deleteResponse = service.delete(deleteRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), deleteResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.CLIENT_CHECK_AND_PUT.getType()) {
                ClientProto.CheckAndPutRequest checkAndPutRequest = ClientProto.CheckAndPutRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.CheckAndPutResponse checkAndPutResponse = service.checkAndPut(checkAndPutRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), checkAndPutResponse.toByteString().toStringUtf8());
                ...
                ClientProto.CommitRequest commitRequest = ClientProto.CommitRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.CommitResponse commitResponse = service.commit(commitRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), commitResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.CLIENT_ROLLBACK.getType()) {
                ClientProto.RollBackRequest rollBackRequest = ClientProto.RollBackRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.RollBackResponse rollBackResponse = service.rollBack(rollBackRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), rollBackResponse.toByteString().toStringUtf8());
                ...
                AdminProto.AddColumnRequest addColumnRequest = AdminProto.AddColumnRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                AdminProto.AddColumnResponse addColumnResponse = service.addColumn(addColumnRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), addColumnResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.ADMIN_DROP_TABLE.getType()) {
                AdminProto.DropTableRequest dropTableRequest = AdminProto.DropTableRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                AdminProto.DropTableResponse dropTableResponse = service.dropTable(dropTableRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), dropTableResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.ADMIN_GET_TABLE_INFO.getType()) {
                AdminProto.GetTableInfoRequest getTableInfoRequest = AdminProto.GetTableInfoRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                AdminProto.GetTableInfoResponse getTableInfoResponse = service.getTableInfo(getTableInfoRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), getTableInfoResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.ADMIN_TRUNCATE_TABLE.getType()) {
                AdminProto.TruncateTableRequest truncateTableRequest = AdminProto.TruncateTableRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                AdminProto.TruncateTableResponse truncateTableResponse = service.truncateTable(truncateTableRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), truncateTableResponse.toByteString().toStringUtf8());
                ...
                ClientProto.TxDeleteRequest txDeleteRequest = ClientProto.TxDeleteRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.TxDeleteResponse txDeleteResponse = service.delete(txDeleteRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), txDeleteResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.TX_GET.getType()) {
                ClientProto.TxGetRequest txGetRequest = ClientProto.TxGetRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.TxGetResponse txGetResponse = service.get(txGetRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), txGetResponse.toByteString().toStringUtf8());
            } else if (callType == CallType.TX_CHECK_AND_PUT.getType()) {
                ClientProto.TxCheckAndPutRequest txCheckAndPutRequest = ClientProto.TxCheckAndPutRequest.parseFrom(ByteUtil.toBytes(rpcRequest.getData()));
                ClientProto.TxCheckAndPutResponse txCheckAndPutResponse = service.checkAndPut(txCheckAndPutRequest);
                rpcResponse = buildRpcResponse(ResponseCode.SUCCESS.getCode(), rpcRequest.getId(), txCheckAndPutResponse.toByteString().toStringUtf8());
            } else {
