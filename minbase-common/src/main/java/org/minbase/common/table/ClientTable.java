...
public interface ClientTable {
    ...
    ColumnValues get(Get get) throws TableNotExistException, ServerException;

    default String get(String key, String column) throws TableNotExistException, ServerException {
        Get get = new Get(key.getBytes(StandardCharsets.UTF_8), Collections.singletonList(ByteUtil.toBytes(column)));
        ColumnValues columnValues = get(get);
        byte[] bytes = columnValues.get(ByteUtil.toBytes(column));
        return bytes == null ? null : new String(bytes);
    }

    void put(Put put) throws TableNotExistException, ServerException;

    default void put(String key, String column, String value) throws TableNotExistException, ServerException {
        Put put = new Put(ByteUtil.toBytes(key), ByteUtil.toBytes(column), ByteUtil.toBytes(value));
        put(put);
    }

    boolean checkAndPut(byte[] checkKey, byte[] column, byte[] checkValue, Put put) throws TableNotExistException, ServerException;
    default boolean checkAndPut(String checkKey, String column, String checkValue, Put put) throws TableNotExistException, ServerException{
        return checkAndPut(ByteUtil.toBytes(checkKey), ByteUtil.toBytes(column), ByteUtil.toBytes(checkValue),  put);
    }
    default boolean checkAndPut(String checkKey, String checkColumn, String checkValue,
                                String key, String column, String value) throws TableNotExistException, ServerException{
        Put put = new Put(ByteUtil.toBytes(key), ByteUtil.toBytes(column), ByteUtil.toBytes(value));
